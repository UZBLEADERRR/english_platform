        ...
        <>
          {loading ? (
            ...
            <div className="flex-1 relative flex flex-col overflow-hidden z-10 w-full h-full bg-black">
              ...
              <div className="w-full px-4 pt-2 pb-4 flex items-center justify-between z-20 shrink-0 bg-gradient-to-b from-black/80 to-transparent">
                <button onClick={() => setSelectedCat('')} className="p-2 -ml-2 rounded-full text-white/70 hover:text-white hover:bg-white/10 transition-colors">
                ...
                <div className="relative">
                  <select
                    ...
                    className="bg-white/10 text-white text-sm font-semibold border border-white/20 rounded-xl pl-4 pr-8 py-2 outline-none appearance-none cursor-pointer min-w-[160px] max-w-[220px] text-center backdrop-blur-md"
                  >
                    {categories.map(cat => (
                      <option key={cat.id} value={cat.id} className="bg-black text-white py-1">
                        {cat.name}
                      </option>
                    ))}
                  </select>
                  <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-white/70 text-[10px]">▼</div>
                </div>
                <div className="w-9" /> {/* Spacer */}
              </div>

              {/* Current card */}
              <div className="flex-1 flex flex-col bg-transparent animate-in slide-in-from-right duration-500" key={currentIndex}>
                {(() => {
